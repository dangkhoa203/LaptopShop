...
namespace APIShopLaptop.Feature.Admin.Summary {
    public class GetTopMainCategory : IEndpoint {
        ...
        public static void MapEndpoint(IEndpointRouteBuilder app) {
            app.MapGet("/api/Admin/Summary/Top_Category", Handler).WithTags("Admin_Summary");
        }
        ...
        private static async Task<IResult> Handler(ApplicationDBContext context) {
            try {
                ...
                foreach (var main in Category) {
                    ...
                    Sales.Add(new SaleDTO(main.Name, UniqueProduct
                                                    .Sum(p => p.Value.OrderDetails
                                                    .Where(d=>d.OrderNavigation.Status==ORDERSTATUS.FINISHED)
                                                    .Sum(d => d.Quantity))));
                    UniqueProduct.Clear();
                }
                var Data = Sales.Where(d=>d.count>0).OrderByDescending(d => d.count).Take(5).ToList();
