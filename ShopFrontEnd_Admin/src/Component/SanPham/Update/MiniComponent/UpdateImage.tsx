...
export default function UpdateImage(props:{id:string}) {
    ...
    const deleteFromList=(index:number)=>{
        const reducedArr = [...productImage];

        reducedArr.splice(index, 1);

        setProductImage(reducedArr);
    }
                    ...
                    {!success ?
                        ...
                        <>
                            <div style={{display:"flex",gap:4,marginBottom:"10px", justifyContent:"center"}}>
                                ...
                                {productImage.length>0 && <Button loading={ADD.isPending} loadingPosition={"end"} endIcon={<ClearIcon/>} variant="contained" onClick={()=>setProductImage([])} color="error">Clear</Button>}
                                {productImage.length>0 && <Button loading={ADD.isPending} loadingPosition={"end"} endIcon={<SaveIcon/>} variant="contained" onClick={()=>ADD.mutate()} color="warning">Lưu</Button>}
                            </div>
                            ...
                            <Grid sx={{border:"2px solid rgb(25, 118, 210)",borderRadius:"5px",backgroundColor:"rgba(25,118,210,0.13)",
                                ...
                                {productImage.map((image:any,index)=>
                                    <Grid sx={{display:"flex",justifyContent:"center"}} size={{sm:12,xs:12,md:6,lg:3}}>
